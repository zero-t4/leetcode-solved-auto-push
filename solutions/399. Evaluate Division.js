function findNodesRelation(graphs, a, b, iterationKey) {
  if (graphs[a].val === b) {
    return 1;
  }
  ...
  for (let i = 0; i < graphs[a].children.length; i++) {
    if (!graphs[a].children[i].visited[iterationKey]) {
      ...
      const foundNodeRelation = findNodesRelation(graphs, graphs[a].children[i].node, b, iterationKey);
...
const calcEquation = (equations, values, queries) => {
  ...
  return queries.map(([a, b], i) => {
    ...
    const foundNode = findNodesRelation(graphs, a, b, i);
